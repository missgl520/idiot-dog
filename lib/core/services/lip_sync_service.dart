...
/// 使用方式：
/// ```dart
/// final service = LipSyncService();
/// service.start();  // 开始监听
/// service.volumeStream.listen((mouthOpenY) {
///   live2dController.setParamFloat('ParamMouthOpenY', mouthOpenY);
/// });
/// service.stop();
/// ```
class LipSyncService {
  ...
  final StreamController<double> _mouthController = StreamController<double>.broadcast();
  ...
  double getLipSyncValue(double volume) {
    ...
    final scaled = _minMouthOpen +
        (volume - _silenceThreshold) / (1.0 - _silenceThreshold) * (_maxMouthOpen - _minMouthOpen);
  ...
  void _tick() {
    ...
    _phase += 2 * math.pi * _frequency / 60;
    _mouthOpen = _minMouthOpen +
        ((_amplitude + 0.05) * math.sin(_phase)).clamp(0, _amplitude + 0.05);
  }
